            ...
            if($_SESSION['usuario']!=="admin"){
                ...
                if($id!=0){
                    ...
                    <form action='#' method='post' enctype='multipart/form-data' class='modificar'>
                        ...
                        <input type='text' name='nombre' value='$datos[nombre]' maxlength='50'>
                        <label for='edad'>Edad</label>
                        <input class='numero' type='number' name='edad' value='$datos[edad]' min='0' max='99'>
                        <label for='usuario'>Usuario</label>
                        <input type='text' name='usuario' value='$datos[usuario]' maxlength='20'>
                        <label for='pass'>Contraseña</label>
                        <input type='password' name='pass' value='$datos[pass]' maxlength='20'>
                ...
                if(isset($_POST['enviar'])){
                    if(!(trim($_POST['nombre'])=="" || $_POST['edad']=="" || trim($_POST['usuario'])=="" || trim($_POST['pass'])=="" || trim($_POST['tlf'])=="")){
                        $tlf=$_POST['tlf'];
                        $correcto=true;
                        $nueva_img=false;
                        ...
                        if($correcto){
                            ...
                            if($cantidad=="0"){
                                $consulta->close();
                                if($nueva_img){
                                    $insercion=$conexion->prepare("update socio set nombre=?,edad=?,usuario=?,pass=?,telefono=?,foto=? where id=$id");
                                    ...
                                    $insercion=$conexion->prepare("update socio set nombre=?,edad=?,usuario=?,pass=?,telefono=?,foto=? where id=$id");
                                    $insercion->bind_param("sissss",$nombre,$edad,$usuario,$pass,$tlf,$nueva_ruta);
